...
import axios from 'axios'; // <-- Koi react-router ka import nahi rahega yahan!
import { IdCard, Calendar, UserCheck, BookOpen, Printer, Palette } from 'lucide-react';

const BASE_URL = 'https://abd-school-backend.onrender.com'; // 👈 exact ye line chipkao bhai

const IDCardStudio = () => {
    // 🎯 ID CARD STUDIO KE ANDAR SE 'const navigate = useNavigate();' KO HATA DEIN!
    ...
    const [timetableGrid, setTimetableGrid] = useState([]); // ✅ FIXED: Variable scope error solved here!
    // 🎭 LIVE MODAL STATE MANAGEMENT ENGINE
    const [showModal, setShowModal] = useState(false);
    const [selectedSlotData, setSelectedSlotData] = useState(null); // Clicked cell metadata
    const [teachersList, setTeachersList] = useState([]); // Database raw faculty array
    const [subjectsList, setSubjectsList] = useState([]); // Master subjects list
    
    // Popup form input trackers
    const [formSubjectId, setFormSubjectId] = useState('');
    const [formTeacherId, setFormTeacherId] = useState('');
    const [formStartTime, setFormStartTime] = useState('09:30');
    const [formEndTime, setFormEndTime] = useState('10:15');

    // 👨‍🏫 FETCH REAL FACULTY LIST FROM BACKEND
    ...
    if (intent === "NAVIGATE") {
        // Standard JS navigation framework loop
        window.location.href = target; 
    } 
    else if (intent === "ACTION") {
        // 🎯 FIX: Undefined modal function error bypass kiya taaki matrix background crash na ho
    ...
    recognition.onresult = async (event) => {
        ...
        try {
            // 🎯 FIX: Port 8000 ko hata kar exact BASE_URL (5000) par redirect kiya
    ...
    // 💾 SUBMIT TIMETABLE SLOT DIRECTLY AS TEXT STRING TO DB
    const handleSaveSlotSubmit = (e) => {
        ...
        const payload = {
            ...
            // 🎯 DIRECT HIT: subject_id aur subject dono me direct text string bhej rahe hain taaki backend filter trap na ho
        ...
        axios.post(`${BASE_URL}/api/timetable/save-slot`, payload)
            .then(res => {
                if (res.data.success) {
                    ...
                    setShowModal(false);
                    fetchTimetableSheet(); // Fresh reload from live DB
                }
            })
            .catch(err => {
    ...
    // 🎯 Is block ko dhyan se dekhiye aur is tarah track par laaiye:
useEffect(() => {
    // Sirf ek baar initial load par chalega, baar-baar loop nahi banayega
    const loadInitialBranding = async () => {
        try {
            const res = await axios.get(`${BASE_URL}/api/settings`);
            if (res.data) {
                // Tightly update static keys without re-triggering parent nodes
    ...
    <div className="w-full bg-gray-50 min-h-screen font-sans">
        {/* 🟢 SINGLE CONTROLS HUB (Sirf ek baar rahega ab) */}
        ...
        <div className="flex items-center justify-end px-6 mb-4 no-print">
            <button
                ...
                className="flex items-center gap-2 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white font-bold py-2.5 px-5 rounded-full shadow-md hover:shadow-lg transition-all active:scale-95 group cursor-pointer"
            >
                <span className="relative flex h-2.5 w-2.5">
                    <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-red-400 opacity-75"></span>
                    <span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-red-500"></span>
                </span>
                <svg xmlns="https://abd-school-backend.onrender.comwww.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor" className="w-4 h-4 group-hover:scale-110 transition-transform">
                ...
                {students.length === 0 ? (
                    ...
                    <>
                        {students.map((student) => {
                            ...
                            return (
                                <div key={student.id || student.admission_no} className={`unbreakable-portrait-card template-${activeStyle.id}`} 
                                     style={{ 
                                         width: '220px', maxWidth: '220px', minWidth: '220px',
                                         height: '320px', maxHeight: '320px', minHeight: '320px',
                                    ...
                                    {/* CONTENT BASE LAYER WITH DYNAMIC WATERMARK LOGO */}
                                    <div style={{ width: '217px', height: '232px', padding: '6px 12px 0 12px', display: 'block', position: 'relative', boxSizing: 'border-box', overflow: 'hidden' }}>
                                        {schoolSettings.logo_url && (
                                            <img 
                                                ...
                                                alt="Watermark" 
                                                style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: '90px', height: 'auto', opacity: '0.045', pointerEvents: 'none', zIndex: '1', display: 'block', mixBlendMode: 'multiply' }} 
                                            />
                                        )}
                        ...
                        {blankPlotsArray.map((_, index) => (
                            <div key={`blank-plot-${index}`} className="strict-blank-plot-placeholder"
                                 style={{ 
                                     width: '220px', maxWidth: '220px', minWidth: '220px',
                                     height: '320px', maxHeight: '320px', minHeight: '320px',
                                     border: '2px dashed #94a3b8', borderRadius: '12px',
                                     display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
                                     background: '#fafafa', position: 'relative', boxSizing: 'border-box'
                                 }}>
                                <div style={{ fontSize: '9px', fontWeight: '900', color: '#cbd5e1', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                                    EMPTY PLOT BOX {filledSlots + index + 1}
                                </div>
...
{/* MODULE 2: 📅 DYNAMIC RELATIONAL CLASS TIMETABLE MATRIX WITH LANDSCAPE PRINT SYSTEM */}
{activeSubTab === 'timetable' && (
    <div className="timetable-active-print-job-parent w-full">
        <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm m-6 printable-timetable-zone timetable-active-print-job">
            {/* 🎯 CONTROLS HEADER ZONE: Is pure section ko saaf aur clean aisa rakhiye */}
<div className="flex flex-wrap justify-between items-center gap-4 mb-6 no-print">
    ...
    <div className="flex items-center gap-3 bg-indigo-50/50 p-2 rounded-xl border border-indigo-100">
        <div>
            ...
            <select 
                className="p-1 text-[11px] font-bold bg-white border border-indigo-200 rounded-lg"
                value={window.configTotalPeriods || 6}
                onChange={(e) => {
                    window.configTotalPeriods = parseInt(e.target.value);
                    setTimetableGrid([]); // Infinite loop bypass matrix click
                }}
            >
        ...
        <div>
            ...
            <select 
                className="p-1 text-[11px] font-bold bg-white border border-indigo-200 rounded-lg"
                value={window.configLunchAfter || 3}
                onChange={(e) => {
                    window.configLunchAfter = parseInt(e.target.value);
                    setTimetableGrid([]); // Safe refresh lock
                }}
            >
            ...
            {/* CONTROLLER FILTERS FOR TIMETABLE GRID */}
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 bg-gray-50 p-4 rounded-xl border border-gray-100 mb-6 no-print">
                ...
                <div className="flex items-end">
                    <button 
                        ...
                        className="w-full bg-green-600 hover:bg-green-700 disabled:bg-gray-200 disabled:text-gray-400 text-white font-black text-xs uppercase tracking-wider p-3 rounded-xl transition shadow-md cursor-pointer flex items-center justify-center gap-2"
                    >
                        🖨️ Print Route Sheet (PDF)
                    </button>
                </div>
            </div>

            {/* 🏢 OFFICIAL PRINT MASTER HEADER (Visible ONLY during document print) */}
            {selectedClass && (
                <div className="print-only-header hidden w-full mb-6 items-center border-b-2 border-slate-300 pb-4">
                    ...
                    <div className="w-2/4 text-center flex flex-col justify-center items-center">
                        ...
                        <span className="text-xs font-black bg-slate-100 text-slate-700 px-4 py-0.5 rounded-full uppercase mt-1.5 border border-slate-200">
                            Class Room: {selectedClass} — Section {selectedSection === 'All' ? 'A' : selectedSection}
                        </span>
                    </div>
            ...
            {/* WEEKLY MATRIX GRID DISPLAY LAYER */}
            {!selectedClass ? (
                <div className="p-12 text-center border border-dashed rounded-xl text-gray-400 font-bold text-xs no-print">
                    📊 Kripya upar se "Target Class Room" select karke load karein taaki weekly time-matrix block active ho sake bhai!
                </div>
            ) : (
        ...
        {Array.from({ length: window.configTotalPeriods || 6 }).map((_, index) => {
            ...
            return (
                <React.Fragment key={index}>
                    ...
                    {/* 🎯 FIXED: Header mein bhi Break ab exact 4th Period ke BAAD hi aayega */}
                                        ...
                                        <React.Fragment key={pIdx}>
                                            <td className="p-2 border-r border-gray-200/60 text-center">
                                                <div 
                                                    onClick={() => {
                                                        loadTeachersData(); 
                                                        setSelectedSlotData({ day, period: p });
                                                        setFormSubjectId(slot ? (slot.subject || slot.subject_name || slot.subject_id || '') : '');
                                                        setFormTeacherId(slot ? slot.teacher_id || '' : '');
                                                        setFormStartTime(slot ? slot.start_time || '09:30' : '09:30');
                                                        setFormEndTime(slot ? slot.end_time || '10:15' : '10:15');
                                                        setShowModal(true);
                                                    }} 
                                                    className={`p-2 rounded-lg shadow-2xs transition-all cursor-pointer group min-w-[110px] border ${slot ? 'bg-indigo-50/40 border-indigo-200 hover:border-indigo-400 print:bg-slate-50/50' : 'bg-white border-gray-200 hover:border-indigo-400'}`}
                                                >
                                                    <div className={`font-black text-[10px] uppercase tracking-wider mb-0.5 ${slot ? 'text-indigo-700' : 'text-gray-400'}`}>
                                                        {slot ? finalSubjectText : 'Empty Slot'}
                                                    </div>
                                                    <div className={`text-[9px] font-bold ${slot ? 'text-slate-700' : 'text-gray-500'}`}>
                                                        {slot ? slot.teacher_name : 'No Faculty'}
                                                    </div>
                                                    <div className="text-[9px] text-slate-500 font-black mt-0.5 bg-slate-100/60 px-1 py-0.5 rounded print:text-slate-600">
                                                        {slot ? `${slot.start_time} - ${slot.end_time}` : '00:00 - 00:00'}
                                                    </div>
                                                </div>
                                            </td>
                                            {/* 🎯 FIXED INTERVAL CONDITION: Body mein bhi Interval exact 4th Period ke baad lock hai */}
{p === (window.configLunchAfter || 4) && (
    <td className="p-2 border-r border-gray-200/60 text-center bg-yellow-50/20 font-black text-yellow-700/80 tracking-widest uppercase text-[9px] print:bg-amber-50/40 align-middle">
        Interval
    </td>
)}
                                        </React.Fragment>
            ...
            {/* DYNAMIC SLOT ASSIGNMENT POPUP MODAL */}
...
{/* ===================================================================== */}
{/* MODULE 3: 👨‍🏫 TEACHER DIRECTORY & REGISTRATION DESK */}
{/* ===================================================================== */}
{activeSubTab === 'teachers' && (
    <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm no-print m-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            {/* RIGHT PANEL: LIVE DIRECTORY LIST */}
                    ...
                    <table className="w-full text-left text-xs font-medium border-collapse">
                        ...
                        <tbody className="divide-y divide-gray-100 text-gray-700 font-semibold">
                            {teachersList.length === 0 ? (
                                <tr>
                                    <td colSpan="4" className="p-6 text-center text-gray-400 font-bold text-xs">⚠️ Koi teacher register nahi hai bhai, left panel se add karo!</td>
                                </tr>
                            ) : (
                                teachersList.map((t) => (
...
{/* HIGH-INTENSITY MATRIC LAYOUT INTERCEPTOR */}
<style>{`
    /* ===================================================================== */
    /* SCREEN VIEWS STYLING (Web UI Normal View Only)                         */
    /* ===================================================================== */
    .master-studio-grid {
    ...
    /* ===================================================================== */
    /* 🖨️ STALWART UNIFORM LANDSCAPE PRINT ENGINE (RESTORES LOGO & HEADER)  */
    /* ===================================================================== */
    @media print {
        ...
        body, html {
            ...
            overflow: hidden !important; /* Block virtual page 2 leakage */
            box-sizing: border-box !important;
            -webkit-print-color-adjust: exact !important; 
            print-color-adjust: exact !important;
        }

        /* 🪪 ID CARDS BLOCK (LANDSCAPE SPREAD - 6 CARDS IN 1 FRAME) */
        .id-cards-active-print-job-parent {
            display: block !important;
            width: 100% !important;
        }
        .id-cards-active-print-job-parent .master-studio-grid {
            ...
            grid-template-rows: repeat(2, 310px) !important; 
        ...
        /* 📅 TIMETABLE CONTAINER VIEWPORT (HEADER & CONTENT SYNCHRONIZATION) */
        ...
        /* Forces your A.B. DIGITAL WORK Header, Logo, and Filters to render properly */
        .print-only-header, 
        div.flex.justify-between.items-center.border-b,
        header { 
            display: flex !important; 
        ...
        .printable-table-wrapper {
            ...
            flex-grow: 1 !important; /* Stretches height smoothly to consume remaining A4 area */
            display: flex !important;
            flex-direction: column !important;
            background: #ffffff !important;
            box-sizing: border-box !important;
        ...
        table {
            ...
            table-layout: fixed !important; /* Strictly aligns headers and cells vertically */
        ...
        /* 🎯 ANTI-CLASH ALIGNMENT RESTORATION BOX */
        td div.group {
            display: block !important; 
        ...
        td div.group div {
            ...
            white-space: nowrap !important; /* Holds strings tight to prevent spilling rows */
            margin: 0 !important;
            padding: 0 !important;
            box-sizing: border-box !important;
        }
